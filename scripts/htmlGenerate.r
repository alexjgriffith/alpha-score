...
prepareTable<-function(loadData,shortNames,fileLocation,sets,pScoreFile,x){
    temp<-sapply(seq(x),scoreList,fileLocation,sets,x,shortNames,pScoreFile)
    apply(
        cbind(
...
htmlGenerateMain<-function(fileLocation,sets,shortNames,imageDirectory,n=3,pScoreFile){
    ...
    M<-sequenceGen(fn=prepareTable,n=n,shortNames=shortNames,loadData,fileLocation,sets,pScoreFile)
